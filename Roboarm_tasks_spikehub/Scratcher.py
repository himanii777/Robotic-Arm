...
SCRATCH_WRIST = PALM_DOWN_WRIST

# Scratcher only needs wrist and yaw for the side-to-side motion.
MOTOR_PORTS = {
    "yaw": "A",
    "wrist": "B",
}
...
def return_default():
    move_to({"wrist": DEFAULT_WRIST, "yaw": 0}, 0.85)
...
def scratch_motion(center_yaw=0, cycles=7):
    center_yaw = clamp(center_yaw, -55, 55)
    move_to({"wrist": SCRATCH_WRIST, "yaw": center_yaw}, 0.75)
    for _ in range(cycles):
        move_to({"yaw": clamp(center_yaw - 18, -70, 70), "wrist": SCRATCH_WRIST}, 0.24)
        move_to({"yaw": clamp(center_yaw + 18, -70, 70), "wrist": SCRATCH_WRIST}, 0.24)
    move_to({"yaw": center_yaw, "wrist": SCRATCH_WRIST}, 0.28)
